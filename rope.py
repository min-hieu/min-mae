# Reference
# https://github.com/facebookresearch/perception_models/blob/main/core/vision_encoder/rope.py
...
class Rope2D:
    ...
    def __init__(self, dim: int, use_cls_token: bool = False, aux_dim: int = None):
        self.dim = dim
    ...
    def update_grid(self, device, grid_h, grid_w, seq_len=None):
        if self.grid_size != (grid_h, grid_w, seq_len):
            self.grid_size = (grid_h, grid_w, seq_len)

            self.rope = self.rope.to(device)

            if self.use_cls_token or self.aux_dim:
            ...
            if self.aux_dim:
                freq = torch.cat(
                    [torch.zeros(self.aux_dim, freq.shape[-1], device=device), freq], dim=0
                )

            self.freq = freq[None, None, ...]

            if seq_len is not None:  # this is for sequence-wise attention
                self.freq = self.freq.repeat(1, 1, seq_len, 1)

        self.freq = self.freq.to(device)

    def __call__(self, q, k):
        q = apply_rotary_emb(self.freq, q)
        k = apply_rotary_emb(self.freq, k)
